   ...
   if linea.strip():
      ...
      if ':' in linea:
         if linea[0] == 'b':
            ...
            linea = linea.split(':')
            puntos_totales = linea[1]
         elif str(linea).find('puntos_jugador') >= 0:
            t = str(linea).rfind('[')+1
            aux = ""
            while linea[t] != ']':
               aux += linea[t]
            ...
            for i in range(len(aux)):
               tm = aux[i].split(':')
               puntos_jugador.append('\t(>= (player_points '+tm[0]+') '+tm[1]+')\n')
         else:
            linea = linea.split(':')
...
for i in range(len(jugador)):
   problema.write(jugador[i]+' ')
problema.write('- player')
problema.write('\n\t ')
# ZONAS
...
problema.write('- type\n\t ')
# TIPO DE JUGADOR
for i in range(len(tipo_jugador)):
   problema.write(tipo_jugador[i]+' ')
problema.write('- type\n')
problema.write(')\n')
problema.write('\n(:init \n')
# RUTAS
...
# LOCALIZACIONES
for i in range(len(jugador)):
   problema.write('\t(looking '+jugador[i]+' '+'Sur)\n')
problema.write('\n')
# MANO VACÍA
problema.write('\n\t(handempty '+jugador[0]+')')
for i in range(len(personajes)):
...
for i in range(len(jugador)):
   problema.write('\t(= (player_points '+jugador[i]+') 0)\n')
problema.write('\n')
# TIPO DE JUGADOR
for i in range(len(jugador)):
   problema.write('\t(type_player '+jugador[i]+' '+tipo_jugador[i]+')\n')
problema.write('\n)')    
problema.write('\n(:goal ')
problema.write('\n\t(and (>= (total_points) '+puntos_totales+')')
for i in range(len(puntos_jugador)):
   problema.write(puntos_jugador[i])
problema.write('\n\t))')

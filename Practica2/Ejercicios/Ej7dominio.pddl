...
(define (domain Ejercicio7)
  ...
  (:action pick-up
         ...
         :precondition (and (location ?pl ?z) (location ?i ?z) 
                            (handempty ?pl) (type_player ?pl Picker))
         :effect
  ...
  ; Acción entregar un objeto a otro jugador
  (:action give-player
         :parameters (?pl1 - player ?pl2 - player ?i - item ?z - zone)
         :precondition (and (location ?pl1 ?z) (location ?pl2 ?z)
                            (has ?pl1 ?i) (type_player ?pl1 Picker) (type_player ?pl2 Dealer) (not (has ?pl1 Zapatillas)) (not (has ?pl1 Bikini)))
         :effect
         (and (not (has ?pl1 ?i)) (has ?pl2 ?i) (handempty ?pl1))
    )

  ; Acción entregar un objeto a un personaje
  (:action give-character
         :parameters (?pl2 - player ?c - character ?i - item ?z - zone)
         :precondition (and (location ?pl2 ?z) (location ?c ?z)
                            (has ?pl2 ?i) (type_player ?pl2 Dealer) (not (has ?pl2 Zapatillas)) (not (has ?pl2 Bikini)))
         :effect
         (and (not (has ?pl2 ?i)) (has ?c ?i) (handempty ?pl2) (increase (total_points) (points ?c ?i)) (increase (magic_pocket ?c) 1) 
              (increase (player_points ?pl2) (points ?c ?i)))
    )
